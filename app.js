...
const ADMIN_MASTER_PASS = "admin123"; // Mật khẩu cứu hộ
...
let currentFilter = 'all'; // Fix lỗi biến undefined
...
// --- HELPER AN TOÀN (CHỐNG SẬP APP KHI THIẾU HTML) ---
...
document.addEventListener('DOMContentLoaded', () => {
    // Chỉ chạy giao diện nếu thư viện đã tải
    if(typeof initTheme === 'function') initTheme();
    if(window.lucide) lucide.createIcons();
    
    startMarquee();
    
    // Luôn hiện Landing Page đầu tiên, ẩn các phần khác
...
window.loadData = async function() {
    ...
    safeSetText('dashboard-marquee', "Đang đồng bộ dữ liệu...");
    ...
    try {
        ...
        // Không logout để tránh loop, chỉ báo lỗi
        alert("Lỗi tải dữ liệu (Vui lòng kiểm tra mạng): " + e.message);
    }
}
...
window.enterSystem = function() {
    ...
    setTimeout(() => {
        ...
        if(u) { 
            // Có user cũ -> Điền sẵn -> HIỆN BẢNG ĐĂNG NHẬP (Không tự submit để tránh lỗi sai pass)
...
window.authLogin = async function() {
    ...
    try {
        ...
        // 1. User chưa tồn tại
        if(!snap.exists()) {
            // Admin cứu hộ: Tự tạo nick nếu đúng mã
            if(ADMIN_LIST.includes(u) && p === ADMIN_MASTER_PASS) {
                ...
                alert("Đã khởi tạo Admin khẩn cấp! Đang đăng nhập...");
                // Tiếp tục đăng nhập
            } else {
        ...
        // Lấy lại data (đề phòng vừa tạo xong)
...
// Giữ nguyên các hàm Modal, Wiki, Capital như bản v31.0
// Tôi sẽ viết gọn lại các hàm phụ trợ để tiết kiệm không gian nhưng vẫn đảm bảo chức năng
