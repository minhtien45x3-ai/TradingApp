...
window.loadData = async function() {
    ...
    if(marquee) marquee.innerText = "🔄 Đang tải dữ liệu từ Cloud...";
    ...
    try {
        ...
        initUI();
        if(marquee) marquee.innerText = "✅ Đã đồng bộ dữ liệu!";
    } catch (error) {
...
window.authRegister = async function() {
    ...
    try {
        ...
        await setDoc(doc(db, "users", user), {
            username: user, created_at: new Date().toISOString(),
            journal: [], wiki: DEFAULT_WIKI, capital: 20000, pairs: DEFAULT_PAIRS, background: 'bg-theme-default'
        });
...
window.authLogin = async function() {
    ...
    if(valid || user) { 
        ...
        localStorage.setItem('min_sys_current_user', user);
...
window.authLogout = function() {
    localStorage.removeItem('min_sys_current_user');
    location.reload();
}
...
window.renderJournalList = function() {
    ...
    list.innerHTML = journalData.map(t => {
        ...
        <tr class="border-b border-slate-200 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/30">
            ...
            <td class="p-4 text-center"><button onclick="deleteEntry('${t.id}')" class="text-slate-400 hover:text-rose-500"><i data-lucide="trash-2" class="w-4 h-4"></i></button></td>
        </tr>`;
    }).join('');
    lucide.createIcons();
    updateDailyPnL();
...
window.renderWikiGrid = function() {
    ...
    const filtered = wikiData.filter(i => 
        ...
        (i.title.toLowerCase().includes(term) || i.code.toLowerCase().includes(term))
    );
    ...
    grid.innerHTML = filtered.map(item => `
        <div class="bg-white dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-emerald-500/50 cursor-pointer group hover:shadow-lg transition" onclick="viewWikiDetail('${item.id}')">
            <div class="flex justify-between mb-2">
                <span class="text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 px-2 py-0.5 rounded">${item.code}</span>
                <span class="text-[10px] text-slate-500 uppercase font-bold">${item.cat}</span>
            </div>
            <div class="h-36 bg-slate-100 dark:bg-slate-900 rounded mb-3 overflow-hidden border border-slate-100 dark:border-slate-700">
                ${item.image ? `<img src="${item.image}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500">` : `<div class="w-full h-full flex items-center justify-center text-slate-400"><i data-lucide="image"></i></div>`}
            </div>
...
window.setBackground = function(theme, save=true) {
    document.body.className = `bg-theme-default text-slate-800 dark:text-slate-200 min-h-screen flex flex-col ${theme}`;
    if (localStorage.theme === 'dark' || (!('theme' in localStorage) && window.matchMedia('(prefers-color-scheme: dark)').matches)) document.body.classList.add('dark');
...
function getCurrentBalance() {
    let pnl = 0;
    journalData.forEach(t => pnl += parseFloat(t.pnl));
...
window.renderCharts = function() {
    ...
    if(ctxEquity && window.Chart) {
        ...
        chartInstances.equity = new Chart(ctxEquity, {
            type: 'line',
            data: { labels, datasets: [{ label: 'Vốn', data: points, borderColor: '#10b981', tension: 0.2, fill: true, backgroundColor: 'rgba(16, 185, 129, 0.1)' }] },
    ...
    if(ctxWinLoss && window.Chart) {
        let win=0, loss=0;
        journalData.filter(t => t.status !== 'OPEN').forEach(t => { if(t.status==='WIN') win++; else loss++; });
        if(chartInstances.winloss) chartInstances.winloss.destroy();
        chartInstances.winloss = new Chart(ctxWinLoss, {
            type: 'doughnut',
...
window.renderPairSelects = function() {
    ...
    document.getElementById('ai-pair-input').innerHTML = html;
    document.getElementById('inp-pair').innerHTML = html;
}
window.renderCategoryFilters = function() {
    const container = document.getElementById('wiki-filter-container');
    const cats = Array.from(new Set(wikiData.map(i=>i.cat))).sort();
